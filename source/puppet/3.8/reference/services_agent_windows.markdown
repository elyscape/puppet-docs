...
For details about invoking the Puppet agent command, see [the puppet agent man page](/references/3.8.latest/man/agent.html).
...
In addition to local logging, Puppet agent will submit a [report][] to the Puppet master after each run. (This can be disabled by setting [`report = false`](/references/3.8.latest/configuration.html#report) in [puppet.conf][].)
