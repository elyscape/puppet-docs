...
> **NOTE:** This page was generated from the Puppet source code on Tue Feb 11 13:03:11 -0800 2014
...
<pre><code>file { 'resource title':
  ...
  <a href="#file-attribute-recurse">recurse</a>                 =&gt; <em># Whether and how to do recursive file management. </em>
...
Valid values are `md5`, `md5lite`, `mtime`, `ctime`, `none`.
...
Valid values are `ssh-dss` (also called `dsa`), `ssh-rsa` (also called `rsa`), `ecdsa-sha2-nistp256`, `ecdsa-sha2-nistp384`, `ecdsa-sha2-nistp521`.
...
* Default for `operatingsystem` == `aix`.
* Supported features: `manages_aix_lam`, `manages_expiry`, `manages_homedir`, `manages_password_age`, `manages_passwords`.
...
* Supported features: `allows_duplicates`, `manages_expiry`, `manages_homedir`, `system_users`.
...
Access the repo via S3. Set this to `absent` to remove it from the file completely.
...
Valid values are `absent`. Values can match `/True|False/`.
