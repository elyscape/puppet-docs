...
**This page is autogenerated; any changes will get overwritten** *(last generated on Thu Jan 10 17:06:54 -0800 2013)*
...
: The specific backend to use for this `package`
  ...
  : Support via `urpmi`.
    ...
    Required binaries: `urpmq`, `rpm`, `urpmi`.  Default for `operatingsystem` == `mandriva, mandrake`.  Supported features: `installable`, `uninstallable`, `upgradeable`, `versionable`.
...
: The specific backend to use for this `service`
  ...
  : Support for AIX's System Resource controller.
    ...
    Required binaries: `/usr/bin/lssrc`, `/usr/bin/refresh`, `/usr/bin/stopsrc`, `/usr/bin/startsrc`.  Default for `operatingsystem` == `aix`.  Supported features: `refreshable`.
...
Provider         | allows duplicates | manages aix lam | manages expiry | manages homedir | manages password age | manages password salt | manages passwords | manages solaris rbac | system users |
---------------- | ----------------- | --------------- | -------------- | --------------- | -------------------- | --------------------- | ----------------- | -------------------- | ------------ |
aix              |                   | *X*             | *X*            | *X*             | *X*                  |                       | *X*               |                      |              |
...
: Whether to manage the home directory when managing the user.
  ...
  delete the home directory when `ensure => absent`. Defaults to `false`.  Valid values are `true`, `false`.
...
: The specific backend to use for this `user`
  ...
  : User management for AIX.
    ...
    Required binaries: `/usr/sbin/lsuser`, `/usr/sbin/lsgroup`, `/usr/bin/chuser`, `/usr/sbin/rmuser`, `/bin/chpasswd`, `/usr/bin/mkuser`.  Default for `operatingsystem` == `aix`.  Supported features: `manages_aix_lam`, `manages_expiry`, `manages_homedir`, `manages_password_age`, `manages_passwords`.

  directoryservice
  : User management on OS X.

    Required binaries: `/usr/bin/dsimport`, `/usr/bin/dscacheutil`, `/usr/bin/plutil`, `/usr/bin/dscl`, `/usr/bin/uuidgen`.  Default for `operatingsystem` == `darwin`.  Supported features: `manages_password_salt`, `manages_passwords`.
  ...
  : User and role management on Solaris, via `useradd` and `roleadd`.
    ...
    Required binaries: `roleadd`, `userdel`, `usermod`, `roledel`, `passwd`, `useradd`, `rolemod`.  Default for `osfamily` == `solaris`.  Supported features: `allows_duplicates`, `manages_homedir`, `manages_password_age`, `manages_passwords`, `manages_solaris_rbac`.
  ...
  : User management via `useradd` and its ilk.  Note that you will need to
    ...
    Required binaries: `userdel`, `usermod`, `chage`, `useradd`.  Supported features: `allows_duplicates`, `manages_expiry`, `manages_homedir`, `system_users`.
...
*This page autogenerated on Thu Jan 10 17:06:56 -0800 2013*
