...
: The specific backend to use for this `user`
  ...
  : User management for AIX.
    ...
    Required binaries: `/usr/bin/chuser`, `/usr/sbin/lsgroup`, `/usr/sbin/lsuser`, `/usr/sbin/rmuser`, `/bin/chpasswd`, `/usr/bin/mkuser`.  Default for `operatingsystem` == `aix`.  Supported features: `manages_aix_lam`, `manages_expiry`, `manages_homedir`, `manages_password_age`, `manages_passwords`.
  ...
  : User and role management on Solaris, via `useradd` and `roleadd`.
    ...
    Required binaries: `usermod`, `roleadd`, `userdel`, `roledel`, `passwd`, `rolemod`, `useradd`.  Default for `osfamily` == `solaris`.  Supported features: `allows_duplicates`, `manages_homedir`, `manages_password_age`, `manages_passwords`, `manages_solaris_rbac`.
...
*This page autogenerated on Mon Feb 04 12:15:09 -0800 2013*
