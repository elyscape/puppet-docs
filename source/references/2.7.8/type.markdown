...
: The specific backend to use for this `service`
    ...
    * **systemd**: Manages `systemd` services using `/bin/systemctl`.  Required binaries: `/bin/systemctl`.      Supported features: `enableable`, `refreshable`.
    * **upstart**: Ubuntu service management with `upstart`.
      ...
      on Ubuntu. For `upstart` documentation, see <http://upstart.ubuntu.com/>.
        Required binaries: `/sbin/status`, `/sbin/start`, `/sbin/initctl`, `/sbin/stop`, `/sbin/restart`.      Supported features: `refreshable`.
    * **windows**:     Support for Windows Service Control Manager (SCM).
        ...
        Default for `operatingsystem` == `windows`.    Supported features: `enableable`, `refreshable`.
...
: The user's password, in whatever encrypted format the local
    ...
    interpolation.  Requires features manages_passwords.
...
: The specific backend to use for this `user`
    ...
    * **aix**: User management for AIX.  Required binaries: `/bin/chpasswd`, `/usr/sbin/lsuser`, `/usr/sbin/rmuser`, `/usr/bin/chuser`, `/usr/sbin/lsgroup`, `/usr/bin/mkuser`.    Default for `operatingsystem` == `aix`.    Supported features: `manages_aix_lam`, `manages_expiry`, `manages_homedir`, `manages_password_age`, `manages_passwords`.
    ...
    * **hpuxuseradd**: User management for HP-UX. This provider uses the undocumented `-F`
        switch to HP-UX's special `usermod` binary to work around the fact that
        its standard `usermod` cannot make changes while the user is logged in.  Required binaries: `/usr/sam/lbin/userdel.sam`, `/usr/sam/lbin/usermod.sam`, `/usr/sbin/useradd`.    Default for `operatingsystem` == `hp-ux`.    Supported features: `allows_duplicates`, `manages_homedir`.
    * **ldap**: User management via LDAP.
        ...
        as it iterates across all existing users to pick the appropriate next one.      Supported features: `manages_passwords`.
    ...
    * **user_role_add**: User and role management on Solaris, via `useradd` and `roleadd`.  Required binaries: `roledel`, `rolemod`, `userdel`, `passwd`, `usermod`, `useradd`, `roleadd`.    Default for `operatingsystem` == `solaris`.    Supported features: `allows_duplicates`, `manages_homedir`, `manages_password_age`, `manages_passwords`, `manages_solaris_rbac`.
    * **useradd**: User management via `useradd` and its ilk.  Note that you will need to
        install Ruby's shadow password library (often known as `ruby-libshadow`)
        if you wish to manage user passwords.  Required binaries: `userdel`, `chage`, `usermod`, `useradd`.      Supported features: `allows_duplicates`, `manages_expiry`, `manages_homedir`, `system_users`.
    * **windows_adsi**: User management for Windows.    Default for `operatingsystem` == `windows`.    Supported features: `manages_homedir`, `manages_passwords`.
...
*This page autogenerated on Tue Dec 13 11:12:10 -0800 2011*
