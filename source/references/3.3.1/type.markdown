...
> **NOTE:** This page was generated from the Puppet source code on Mon Oct 07 13:54:49 -0700 2013
...
<pre><code>cron { 'resource title':
  ...
  <a href="#cron-attribute-target">target</a>      =&gt; <em># The username that will own the cron entry...</em>
  <a href="#cron-attribute-user">user</a>        =&gt; <em># The user to run the command as.  This user must...</em>
...
<pre><code>exec { 'resource title':
  ...
  <a href="#exec-attribute-returns">returns</a>     =&gt; <em># The expected return code(s).  An error will be...</em>
...
<pre><code>file { 'resource title':
  ...
  <a href="#file-attribute-ensure">ensure</a>                  =&gt; <em># Whether to create files that don't currently...</em>
  ...
  <a href="#file-attribute-recurse">recurse</a>                 =&gt; <em># Whether and how deeply to do recursive...</em>
  <a href="#file-attribute-recurselimit">recurselimit</a>            =&gt; <em># How deeply to do recursive management.  Values...</em>
...
<pre><code>group { 'resource title':
  ...
  <a href="#group-attribute-forcelocal">forcelocal</a>           =&gt; <em># Forces the mangement of local accounts when...</em>
...
being managed by some other NSS  Requires features libuser.
...
<pre><code>mount { 'resource title':
  ...
  <a href="#mount-attribute-options">options</a>     =&gt; <em># Mount options for the mounts, as they would...</em>
...
<pre><code>package { 'resource title':
  ...
  <a href="#package-attribute-configfiles">configfiles</a>       =&gt; <em># Whether configfiles should be kept or replaced.  </em>
  ...
  <a href="#package-attribute-source">source</a>            =&gt; <em># Where to find the actual package.  This must be...</em>
...
Required binaries: `/sw/bin/apt-get`, `/sw/bin/apt-cache`, `/sw/bin/fink`, `/sw/bin/dpkg-query`.  Supported features: `holdable`, `installable`, `purgeable`, `uninstallable`, `upgradeable`, `versionable`.
...
Required binaries: `/usr/sbin/pkg_add`, `/usr/sbin/pkg_delete`, `/usr/sbin/pkg_info`.  Supported features: `installable`, `purgeable`, `uninstallable`.
...
Required binaries: `/usr/sbin/nimclient`, `rpm`, `/usr/bin/lslpp`.  Supported features: `installable`, `uninstallable`, `upgradeable`, `versionable`.

<h4 id="package-provider-openbsd">openbsd</h4>

OpenBSD's form of `pkg_add` support.

Required binaries: `pkg_add`, `pkg_delete`, `pkg_info`.  Default for `operatingsystem` == `openbsd`.  Supported features: `install_options`, `installable`, `purgeable`, `uninstall_options`, `uninstallable`, `versionable`.
...
Required binaries: `urpmq`, `urpme`, `rpm`, `urpmi`.  Default for `operatingsystem` == `mandriva, mandrake`.  Supported features: `installable`, `purgeable`, `uninstallable`, `upgradeable`, `versionable`.
...
Required binaries: `/usr/bin/zypper`.  Supported features: `install_options`, `installable`, `uninstallable`, `upgradeable`, `versionable`.
...
<pre><code>resources { 'resource title':
  ...
  <a href="#resources-attribute-purge">purge</a>              =&gt; <em># Purge unmanaged resources.  This will delete any </em>
...
<pre><code>schedule { 'resource title':
  <a href="#schedule-attribute-name">name</a>        =&gt; <em># <strong>(namevar)</strong> The name of the schedule.  This name is used to...</em>
  <a href="#schedule-attribute-period">period</a>      =&gt; <em># The period of repetition for a resource. The...</em>
...
<pre><code>selboolean { 'resource title':
  ...
  <a href="#selboolean-attribute-value">value</a>      =&gt; <em># Whether the the SELinux boolean should be...</em>
  # ...plus any applicable <a href="./metaparameter.html">metaparameters</a>.
}</code></pre>
...
Required binaries: `/usr/sbin/svcadm`, `/usr/bin/svcs`, `/usr/sbin/svccfg`.  Default for `osfamily` == `solaris`.  Supported features: `enableable`, `refreshable`.
...
Required binaries: `/usr/sbin/mkitab`, `/usr/bin/refresh`, `/usr/sbin/rmitab`, `/usr/bin/stopsrc`, `/usr/sbin/chitab`, `/usr/bin/startsrc`, `/usr/bin/lssrc`, `/usr/sbin/lsitab`.  Default for `operatingsystem` == `aix`.  Supported features: `enableable`, `refreshable`.
...
Required binaries: `/sbin/initctl`, `/sbin/start`, `/sbin/restart`, `/sbin/stop`, `/sbin/status`.  Default for `operatingsystem` == `ubuntu`.  Supported features: `enableable`, `refreshable`.
...
<pre><code>sshkey { 'resource title':
  ...
  <a href="#sshkey-attribute-key">key</a>          =&gt; <em># The key itself; generally a long string of hex...</em>
...
<pre><code>stage { 'resource title':
  <a href="#stage-attribute-name">name</a> =&gt; <em># <strong>(namevar)</strong> The name of the stage. This will be used as the...</em>
  # ...plus any applicable <a href="./metaparameter.html">metaparameters</a>.
}</code></pre>
...
<pre><code>user { 'resource title':
  ...
  <a href="#user-attribute-forcelocal">forcelocal</a>           =&gt; <em># Forces the mangement of local accounts when...</em>
...
expire, you can pass the special value `absent`.  Valid values are `absent`.  Values can match `/^\d{4}-\d{2}-\d{2}$/`.  Requires features manages_expiry.
...
is used in OS X  Requires features manages_password_salt.
...
OS X  Requires features manages_password_salt.
...
Required binaries: `/usr/bin/dscl`, `/usr/bin/uuidgen`, `/usr/bin/dsimport`, `/usr/bin/dscacheutil`, `/usr/bin/plutil`.  Default for `operatingsystem` == `darwin`.  Supported features: `manages_password_salt`, `manages_passwords`.
...
Required binaries: `pw`.  Default for `operatingsystem` == `freebsd, dragonfly`.  Supported features: `allows_duplicates`, `manages_expiry`, `manages_homedir`, `manages_passwords`.

<h4 id="user-provider-user_role_add">user_role_add</h4>

User and role management on Solaris, via `useradd` and `roleadd`.

Required binaries: `passwd`, `roledel`, `rolemod`, `usermod`, `userdel`, `roleadd`, `useradd`.  Default for `osfamily` == `solaris`.  Supported features: `allows_duplicates`, `manages_homedir`, `manages_password_age`, `manages_passwords`, `manages_solaris_rbac`.
...
Required binaries: `chage`, `luseradd`, `usermod`, `userdel`, `useradd`.  Supported features: `allows_duplicates`, `manages_expiry`, `manages_homedir`, `system_users`.
...
<pre><code>yumrepo { 'resource title':
  ...
  <a href="#yumrepo-attribute-enabled">enabled</a>         =&gt; <em># Whether this repository is enabled, as...</em>
  ...
  <a href="#yumrepo-attribute-failovermethod">failovermethod</a>  =&gt; <em># The failover methode for this repository; should </em>
  ...
  <a href="#yumrepo-attribute-proxy">proxy</a>           =&gt; <em># URL to the proxy server for this repository. Set </em>
  ...
  <a href="#yumrepo-attribute-s3_enabled">s3_enabled</a>      =&gt; <em># Access the repo via S3. Set this to `absent` to...</em>
...
Access the repo via S3. Set this to `absent` to remove it from the file completely.  Valid values are `absent`.  Values can match `/^(0|1)$/`.
...
Set this to `absent` to remove it from the file completely.  Valid values are `absent`.  Values can match `/True|False/`.
...
<pre><code>zone { 'resource title':
  ...
  <a href="#zone-attribute-ip">ip</a>           =&gt; <em># The IP address of the zone.  IP addresses must...</em>
