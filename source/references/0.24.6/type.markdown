...
<p><strong>This page is autogenerated; any changes will get overwritten</strong> <em>(last generated on Thu Feb 24 17:23:22 -0800 2011)</em></p>
...
<p>The basic property that the object should be in.  Valid values are <code>absent</code>, <code>present</code>.</p>
...
checksum changes, for instance.  Valid values are <code>timestamp</code>, <code>mtime</code>, <code>nosum</code>, <code>md5</code>, <code>time</code>, <code>md5lite</code>.  Values can also match <code>(?-mix:^\{md5|md5lite|timestamp|mtime|time\})</code>.</p>
...
and links corresponding to each file.  Valid values are <code>absent</code> (also called <code>false</code>), <code>file</code>, <code>link</code>, <code>directory</code>, <code>present</code>.  Values can also match <code>(?-mix:.)</code>.</p>
...
<p>The basic state that the object should be in.  Valid values are <code>absent</code>, <code>present</code>.</p>
...
<dd><p>not to work currently.  Required binaries: <code>nireport</code>, <code>niutil</code>, <code>mount</code>, <code>umount</code>, <code>df</code>.    Default for <code>operatingsystem</code> == <code>darwin</code>.</p>
</dd>
</dl>
</li>
...
<p>The basic property that the object should be in.  Valid values are <code>absent</code>, <code>purged</code>, <code>present</code>.</p>
...
fstab and mount it.  Valid values are <code>absent</code>, <code>mounted</code>, <code>present</code> (also called <code>unmounted</code>).</p>
...
'normal' system files.  Valid values are <code>absent</code>, <code>purged</code>, <code>latest</code>, <code>present</code> (also called <code>installed</code>).  Values can also match <code>(?-mix:.)</code>.</p>
...
<li><p><strong>yum</strong>: Support via <code>yum</code>.  Required binaries: <code>yum</code>, <code>python</code>, <code>rpm</code>.    Default for <code>operatingsystem</code> == <code>fedoracentosredhat</code>.    Supported features: <code>installable</code>, <code>uninstallable</code>, <code>upgradeable</code>, <code>versionable</code>.</p>
</li>
</ul>
...
<p>Whether the the SELinux boolean should be enabled or disabled.  Valid values are <code>off</code>, <code>on</code>.</p>
...
<li><dl>
<dt><strong>daemontools</strong>: Daemontools service management.</dt>
<dd><p>This provider manages daemons running supervised by D.J.Bernstein daemontools.
...
<p>or this can be overriden in the service resource parameters:
service {</p>
<blockquote>
<dl>
<dt>&quot;myservice&quot;:</dt>
<dd><p>provider =&gt; &quot;daemontools&quot;, path =&gt; &quot;/path/to/daemons&quot;;</p>
</dd>
</dl>
</blockquote>
<p>}</p>
<dl>
<dt>This provider supports out of the box:</dt>
<dd><ul>
<li>start/stop (mapped to enable/disable)</li>
<li>enable/disable</li>
<li>restart</li>
<li>status  Required binaries: <code>/usr/bin/svstat</code>, <code>/usr/bin/svc</code>.      Supported features: <code>enableable</code>, <code>refreshable</code>.</li>
</ul>
</dd>
</dl>
</dd>
...
<dd><p>is that this supports service enabling and disabling via <code>update-rc.d</code>.  Required binaries: <code>/usr/sbin/update-rc.d</code>.    Default for <code>operatingsystem</code> == <code>debianubuntu</code>.    Supported features: <code>enableable</code>, <code>refreshable</code>.</p>
</dd>
</dl>
</li>
<li><dl>
...
<dd><p>management; uses <code>chkconfig</code> for service enabling and disabling.  Required binaries: <code>/sbin/chkconfig</code>, <code>/sbin/service</code>.    Default for <code>operatingsystem</code> == <code>redhatfedorasusecentossles</code>.    Supported features: <code>enableable</code>, <code>refreshable</code>.</p>
</dd>
</dl>
</li>
<li><dl>
<dt><strong>runit</strong>: Runit service management.</dt>
<dd><p>This provider manages daemons running supervised by Runit.
It tries to detect the service directory, with by order of preference:</p>
<blockquote>
<ul>
<li>/service</li>
...
<dl>
<dt>&quot;myservice&quot;:</dt>
<dd><p>provider =&gt; &quot;runit&quot;, path =&gt; &quot;/path/to/daemons&quot;;</p>
</dd>
</dl>
</blockquote>
<p>}</p>
...
<li>enable/disable</li>
<li>restart</li>
<li>status  Required binaries: <code>/usr/bin/sv</code>.      Supported features: <code>enableable</code>, <code>refreshable</code>.</li>
</ul>
</dd>
</dl>
</dd>
...
<li><dl>
<dt><strong>smf</strong>: Support for Sun's new Service Management Framework.  Starting a service</dt>
...
<p>The basic state that the object should be in.  Valid values are <code>absent</code>, <code>role</code>, <code>present</code>.</p>
...
<li><dl>
<dt><strong>netinfo</strong>: User management in NetInfo.  Note that NetInfo is not smart enough to fill in default information</dt>
<dd><p>for users, so this provider will use default settings for home (<code>/var/empty</code>), shell (<code>/usr/bin/false</code>),
comment (the user name, capitalized), and password ('<strong>****</strong>').  These defaults are only used when the user is created.
...
enable us to set a password.  Required binaries: <code>nireport</code>, <code>niutil</code>.    Default for <code>operatingsystem</code> == <code>darwin</code>.    Supported features: <code>manages_passwords</code>.</p>
</dd>
</dl>
</li>
...
<li><p><strong>user_role_add</strong>: User management inherits <code>useradd</code> and adds logic to manage roles on Solaris using roleadd.  Required binaries: <code>rolemod</code>, <code>userdel</code>, <code>usermod</code>, <code>roleadd</code>, <code>roledel</code>, <code>useradd</code>.    Default for <code>operatingsystem</code> == <code>solaris</code>.    Supported features: <code>allows_duplicates</code>, <code>manages_homedir</code>, <code>manages_solaris_rbac</code>.</p>
</li>
...
Set this to 'absent' to remove it from the file completely  Valid values are <code>absent</code>.  Values can also match <code>(?-mix:.*)</code>.</p>
...
values are '0', and '1'.
Set this to 'absent' to remove it from the file completely  Valid values are <code>absent</code>.  Values can also match <code>(?-mix:(0|1))</code>.</p>
...
Set this to 'absent' to remove it from the file completely  Valid values are <code>absent</code>.  Values can also match <code>(?-mix:roundrobin|priority)</code>.</p>
...
<p>Set this to 'absent' to remove it from the file completely  Valid values are <code>absent</code>.  Values can also match <code>(?-mix:(0|1))</code>.</p>
...
Set this to 'absent' to remove it from the file completely  Valid values are <code>absent</code>.  Values can also match <code>(?-mix:[0-9]+)</code>.</p>
...
Set this to 'absent' to remove it from the file completely  Valid values are <code>absent</code>.  Values can also match <code>(?-mix:[1-9][0-9]?)</code>.</p>
...
used to stop zones.  Valid values are <code>absent</code>, <code>configured</code>, <code>installed</code>, <code>running</code>.</p>
...
appropriate provider for your platform.  Available providers are:</p>
<ul>
<li><strong>solaris</strong>: Provider for Solaris Zones.  Required binaries: <code>/usr/sbin/zonecfg</code>, <code>/usr/sbin/zoneadm</code>.    Default for <code>operatingsystem</code> == <code>solaris</code>.</li>
</ul>
...
<hr />
<p><em>This page autogenerated on Thu Feb 24 17:23:23 -0800 2011</em></p>
