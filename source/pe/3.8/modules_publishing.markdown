...
>**DEPRECATION WARNING** As of Puppet 3.6 the Modulefile has been deprecated in favor of the metadata.json file. We strongly suggest that you use `puppet module generate` to create new modules, as this command now includes a dialog for creating your metadata.json. You can read more about how to [deal with your Modulefile](#build-your-modulefile) below.
...
Your metadata.json will look something like this:
...
* `name` --- REQUIRED. The **full name** of your module, including the username (such as "username-module" --- [see note above](#a-note-on-module-names)).
...
* `operatingsystem_support` --- A list of operating system compatibilities for your module. See [Operating system compatibility in metadata.json](#operating-system-compatibility-in-metadatajson) below for more details.
* `tags` --- A list of key words that will help others find your module (not case sensitive)(for example, [“msyql”, “database”, “monitoring”]). Tags cannot contain whitespace. We recommend using four to six tags.

#####DEPRECATED

* `checksums`--- File checksums generated by the Puppet module tool (PMT) as part of `puppet module build`. **You should remove this field from your metadata.json, if it's present.**
* `description` --- Legacy field used to describe the purpose of the module. You should use the `summary` field instead.
* `types` --- Resource type documentation generated by the PMT as part of `puppet module build`. **You should remove this field from your metadata.json, if it's present.**
...
In order for your module to be successfully uploaded to and displayed on the Forge, your metadata.json file will minimally need to have the following [fields](#fields-in-metadatajson): name, version, source, author, license, summary, and dependencies.
...
~~~
    # puppet module build  /etc/puppetlabs/puppet/environments/production/modules/mymodule
    Building /etc/puppetlabs/puppet/environments/production/modules/mymodule for release
    /etc/puppetlabs/puppet/environments/production/modules/mymodule/pkg/examplecorp-mymodule-0.0.1.tar.gz
~~~
...
1. Run `# puppet module build <MODULE DIRECTORY>`. The PMTl will merge your Modulefile and metadata.json files and issue a deprecation warning.
...
------
Now that you have a compiled `tar.gz` package, you can upload it to the Puppet Forge.

>*Note:* Your tarball must be 10MB or less. There is currently no command line tool for publishing; you must use the Puppet Forge's web interface.
...
2. Click on "Publish" in the upper right corner of the screen.
